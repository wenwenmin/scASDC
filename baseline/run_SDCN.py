...
class AE(nn.Module):
    ...
    def __init__(self, n_enc_1, n_enc_2, n_enc_3, n_dec_1, n_dec_2, n_dec_3,
        ...
        # todo:这里的z_layer后面可以参考scDSC变成三层，维数逐渐减小
        self.z_layer = Linear(n_enc_3, n_z)
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        ari_li.append(ari)
        f1_li.append(f1)
    np.savez("results/SDCN_{}.npz".format(dataname), ARI=ari_li, NMI=nmi_li, ACC=acc_li, f1=f1_li)

    loaded_data = np.load('results/SDCN_{}.npz'.format(dataname))
    ...
    np.savetxt('results/SDCN_{}_res.csv'.format(dataname), combined_array, delimiter=',')
